class Ball {
            ...
            if ((padX - r) < (ballX) && (ballX) < (padX + pad.w + r)) {
                ...
                if ((padY - r) < ballY && ballY < (padY + pad.h + r)) {
                    ...
                    if (a > -PI/2 && a < PI/2) {
                      ...
                      bounceSound.play(); // bouncing after hitting to paddle
                    } else {
                        this.vel.rotate(PI);
                        let a = this.vel.heading();
    ...
    update() {
        ...
        if (this.pos.y + this.r >= height || this.pos.y - this.r <= 0) {
          ...
          // sound processing for dartSound
          // changing the effect if the ball hits the ground
          let myFilter = new p5.BandPass();
          let myFreq =  map(this.pos.y,0,width,100,6000);
          myFilter.freq(myFreq);
          dartSound.disconnect();
          dartSound.connect(myFilter);
          dartSound.play();
        }
    ...
    show() {
      // new condition for checking wowSound status
         if(wowSound.isPlaying())
    {
